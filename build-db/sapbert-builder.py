...
def get_name_id_dict(file_path: str) -> Dict[str, str]:
    """Load name to ID mapping with progress tracking"""
    result = {}
    try:
        with open(file_path, mode='r', newline='') as csvfile:
            total = sum(1 for _ in csvfile) - 1
            ...
            reader = csv.DictReader(csvfile)
            for row in tqdm(reader, total=total, desc=f"Loading {Path(file_path).name}"):
                result[row['Name']] = ast.literal_eval(row['ID'])[0]
    except Exception as e:
        logger.error(f"Error loading {file_path}: {e}")
        raise
    return result
...
def iter_files(np_file: str, name_id_file: str, id_type_file: str,
               type_id_dict: Dict, name_id_dict: Dict,
               start_index: int = 0) -> Generator:
    """Generate documents from files with optional start index for resume"""
    ...
    with open(name_id_file) as name_id_csv:
        ...
        for row in tqdm(reader, total=total_rows - start_index,
            ...
            try:
                curies = ast.literal_eval(row['ID'])
                vector = np_arr[index]
...
async def main(config: Config):
    ...
    try:
        ...
        for folder_idx, folder in enumerate(config.folders[start_folder_idx:], start=start_folder_idx):
            ...
            name_id_file = os.path.join(folder_path, "name_ids.csv")
            ...
            # Load metadata
            logger.info(f"Loading metadata from {folder}")
            type_id_dict = get_id_type_dict(id_type_file)
            id_name_dict = get_name_id_dict(name_id_file)
            ...
            for file_idx, (name_id, emb) in enumerate(
                ...
                try:
                    counter = await client.populate_index(
                        iter_files(emb, name_id, id_type_file, type_id_dict, id_name_dict),
                        counter,
                        shutdown_handler
                    )
...
def parse_args():
    ...
    parser.add_argument("--prefetc-size", type=int, default=5, help="Prefetch buffer size")
